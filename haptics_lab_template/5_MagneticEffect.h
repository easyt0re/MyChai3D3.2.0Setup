...
void MagneticEffect::updateHaptics(cGenericHapticDevice* hapticDevice, double timeStep, double totalTime)
{
    ...
    double d = cAbs(y);
    double sgn = cSign(y);
    double forceFieldWidthHalf = 0.005;
    bool forceStatus;
    bool proxyStatus;
    if(d >= forceFieldWidthHalf)
    {
        forceStatus = false;
    }
    else
    ...
    if(forceStatus)
    {
        double forceMag = -cCosDeg(d/forceFieldWidthHalf*180.0) + 1;
        force.y(3 * forceMag * -sgn);
    }
//    if(proxyStatus)
//    {
//        // proxy idea
